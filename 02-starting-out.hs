...
isPalindrome l
    | n < 2     = True
    | otherwise = if (l !! 0) == (l !! (n - 1)) then isPalindrome (tail (init l)) else False
    where n = length l
...
ziplike xs ys
    | n == 0    = []
    | otherwise = (xs !! 0, ys !! 0) : ziplike (tail xs) (tail ys)
...
insertElem x k l = (take k l) ++ (x : (take (length l - k) l))
